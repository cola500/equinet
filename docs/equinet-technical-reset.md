...
- **Kanonisk demo-seed:** `scripts/seed-demo-provider.ts` (lokal + staging). Skapar demo-personerna bakom inloggningsknapparna (provider `erik.jarnfot@demo.equinet.se`, kund `lisa.andersson@gmail.com`) samt demo-stallet **Stall Solbacken**. Körs via `npm run db:seed:demo-provider` (lokalt) och `seed-staging-demo.sh` (staging, guardad). **Lägg ny demo-grunddata här — på ett ställe.**
- **Legacy/local-only:** `prisma/seed-demo.ts`. Idempotent för tjänster/kunder/hästar (upsert/findFirst), bokningar bara om inga `@demo.equinet.se`-bokningar finns. `--reset` raderar och återskapar. Seedar andra personer (provider@example.com + anna/erik/sofia) och används **inte** på staging.
...
4. **`prisma/seed-demo.ts`** — **legacy/local-only** (kanonisk demo = `scripts/seed-demo-provider.ts`). Lägg ingen ny demo-grunddata här. Ändringar utan koordination kan bryta `db:seed:demo:reset` lokalt.
