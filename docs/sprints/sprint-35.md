---
title: "Sprint 35: Messaging Slice 1 (MVP)"
...
# Sprint 35: Messaging Slice 1 (MVP)
...
Stories är sekventiella pga beroende: S35-0 (schema) → S35-1 (kund-flöde) → S35-2 (leverantör-flöde) → S35-3 (push).

En session räcker. Om parallellt önskas: S35-3 (push) kan köras av iOS-session efter S35-2 är mergad.

---

## Stories

### S35-0: Plan-review av Conversation-domän
...
- [ ] Prisma schema-utkast inkluderat (faktisk migration i S35-1)
- [ ] Epic-dokumentet uppdaterat med länk till arkitekturbeslut

---

### S35-1: Kund kan skicka meddelande till leverantör om bokning

**Prioritet:** 1
**Effort:** 1-1.5 dag
**Domän:** webb (nytt Prisma schema, ny domän, ny API, kund-UI)

Första halvan av MVP-värdet: kunden kan initiera och skicka text-meddelanden kopplade till en bokning. Meddelandet sparas och blir läsbart av leverantören (i S35-2).
...
- Lägg till `Conversation` + `Message`-modeller enligt S35-0 design
...
**Reviews:** tech-architect (plan om S35-0 ändrades), security-reviewer (kod), code-reviewer
...
**Aktualitet verifierad:** S35-1 levererar grunden. Denna story adderar leverantör-sidan.
...
- Skriv-fält + skicka-knapp (reuses API från S35-1, men tillåter sender=PROVIDER)
...
- E2E: leverantör loggar in → ser inkorg-badge → öppnar tråd → svarar → kund ser svaret (återanvänd S35-1 kund-flöde)
...
**Reviews:** code-reviewer (trivial om S35-1/2 etablerat mönstret)
...
```
S35-0 (2-3h, plan-review) -> S35-1 (1-1.5 dag, kund) -> S35-2 (1.5-2 dag, leverantör) -> S35-3 (0.5 dag, push)
```

**Total effort:** ~4-5 dagar (matchar epic-skattningen).

**Parallelliseringsmöjlighet:** S35-3 kan köras parallellt av iOS-session EFTER S35-2 är mergad.

## Risker och oklarheter

1. **Domän-scope:** Hör `Conversation` till Booking-domänen eller är det egen kärndomän? S35-0:s tech-architect-review avgör. Om underordnad Booking: enklare, men kopplad för nära. Om egen: mer kod, men följer Slice 5-förändringar bättre.
...
- [ ] S35-0: domänarkitektur godkänd av tech-architect + security-reviewer
- [ ] S35-1: kund kan skicka meddelande, E2E grön
- [ ] S35-2: leverantör kan läsa + svara, inkorg fungerar, E2E grön
- [ ] S35-3: push skickas vid nytt meddelande
