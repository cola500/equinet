...
*(ingen aktiv sprint — väntar på S50)*
...
| iOS auth-polish continued (S49-1 review-follow-up) | 1.5-2h | **6 minor-fynd från code-reviewer + ios-expert på S49-1 PR #266**: (1) Banner-UI för exchange-fel är deferred — `webCookieExchangeFailed` har TODO-kommentar, AC #2 från sprint-49 delvis uppfyllt; (2) Tester verifierar inte cookie-injection faktiskt — DI-parametern `cookieStorage` används inte i test, befintlig "does not crash"-kommentar kvar; (3) `filterCookies` hasSuffix edge-case — matchar `evil.equinet.vercel.app.evil.com`, fix: kontrollera att domän börjar med `.` eller använd `HTTPCookie.isDomainMatch`; (4) Retry särskiljer inte 4xx/5xx — 401 retryas 3 ggr fast det är permanent fel, fix: short-circuit vid 4xx (utom 408/429); (5) `authStateChanges`-stream dör tyst vid nätverksfel — ingen reconnect-logik, bara warning-log; (6) Test-fragilitet: `Task.sleep 300ms` i `testLogout_clearsCookiesFromInjectedStore` — använd XCTestExpectation. Alla defense-in-depth eller robustness. Inga blockers. |
