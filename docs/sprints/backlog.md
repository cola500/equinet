...
| **"Byt lösenord"-funktion saknas** | 0.5 dag | Inloggade användare kan inte ändra sitt lösenord från profilen/inställningar. Enda vägen idag är "Glömt lösenord"-flödet (som dessutom är trasigt pga fire-and-forget) eller manuellt via Supabase Dashboard. Bygg `/api/auth/change-password`-route som tar gammalt + nytt lösenord, verifierar mot Supabase Auth, uppdaterar via `auth.admin.updateUserById`. UI under Inställningar/Profil. Hittad 2026-04-30 vid felsökning av password reset. |
