...
|------|--------|-------------|
| **🔴 INCIDENT: Offline E2E Smoke RLS-grant-fel (blockerar prod-release 2026-06-12)** | 1-2h | **Reproducerbart** (2 körningar, ej flake) på PR #404 (staging→main). `e2e/offline-mutations.spec.ts:230` timeoutar: bokningskortet renderar aldrig pga **`permission denied for table Booking/Notification/Service`** — RLS-scopade Supabase-klienten saknar SELECT-grant på dessa tabeller i offline-E2E:s lokala Supabase. **Inte orsakat av messaging-releasen** (0 messaging-träffar i loggen). Trolig rotorsak: grant-/RLS-setup applicerades inte i CI:s lokala Supabase — kopplat till tyst-trasiga `npm run setup` auth-triggers-steget (`prisma db execute --file supabase/auth-triggers.sql` utan `--schema`/`--url`, se separat backlog-rad). Passerade på #398 → grant-setup är icke-deterministisk. **Åtgärd:** (1) fixa `--schema` i setup-scriptet, (2) verifiera att GRANT/RLS för authenticated-rollen på Booking/Notification/Service appliceras i offline-E2E-bootstrap, (3) kör om #404. Blockerar Feature Flag Retirement slice 1+2 prod-release tills löst. |
