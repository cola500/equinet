...
export class RouteAnnouncementNotifier {
  ...
  async notifyFollowersOfNewRoute(routeOrderId: string): Promise<void> {
    ...
    for (const follower of followers) {
      ...
      const message = `${routeOrder.provider.businessName} har annonserat nya tider i ${routeOrder.municipality} (${dateStr}). Tjänster: ${serviceNames}`

      // In-app notification
      await this.notificationService.createAsync({
        userId: follower.userId,
        type: NotificationType.ROUTE_ANNOUNCEMENT_NEW,
        message,
      ...
      try {
        ...
        await this.emailService.send({
          to: follower.email,
          subject: `Ny ruttannons i ${routeOrder.municipality} - ${routeOrder.provider.businessName}`,
          html,
          text,
        })
