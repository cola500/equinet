...
- **Staging-arkitektur (2026-05-06)**: Custom domain `equinet-staging.johanlindengard.com` (separat från prod `equinet.johanlindengard.com`). Supabase-projekt `zzdamokfeenencuggjjp` (Frankfurt) — separat från prod `xybyzflfxnqqyxnvjklv` (Zurich), men **delat med ett annat, obesläktat projekt** (se notis nedan). Egen `DATABASE_URL`/`DIRECT_DATABASE_URL`/`APP_URL` per environment via Vercel env-rader. Custom Access Token Hook installerad i båda. Se `docs/operations/staging-environment-setup.md`.
- **`zzdamokfeenencuggjjp` är ett medvetet DELAT Supabase-projekt, inte stale infra**: Projektet heter "Slot Machine" i Supabase-dashboarden och äger scheman för ett helt annat, obesläktat projekt (`signals`, `lakers`, `tippliga`). Equinet staging samexisterar avsiktligt i samma projekt sedan starten — det är INTE fel projekt-ID, inte en rest från en gammal migrering, och inget som ska "städas upp". Byt aldrig detta projekt-ID och ta aldrig bort Equinet-tabeller därifrån utan att fråga Johan. Säkerhets-/RLS-avvikelser på Equinet-tabellerna i detta projekt är riktiga staging-fynd och ska hanteras som sådana — bortförklara dem aldrig som brus från "fel projekt".
...
- **Routa via /dashboard, hårdkoda ALDRIG roll-redirect**: Efter login → `router.push("/dashboard")`; `/dashboard` (server) redirectar per `userType` (kund → `/hem`, provider → alltid `/provider/calendar`). Duplicera aldrig roll-routing i login eller demo-knappar.
