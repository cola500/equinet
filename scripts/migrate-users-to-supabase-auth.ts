 ...
 *
 * Reads password hashes from PoC auth.users (via pg) and creates users
 * in prod auth.users (via Supabase Admin API) with the same hashes.
 *
 * Background: S15-1 dropped the passwordHash column from public.User on prod.
 * The hashes were migrated to PoC auth.users in S11-2.
 *
 * Usage:
 ...
 *
 * Env files:
 *   .env          - PoC DIRECT_DATABASE_URL (commented out, parsed manually)
...
// Prod credentials from .env.supabase
...
// PoC database URL -- read from .env (commented out line)
function getPocDatabaseUrl(): string {
  const envContent = readFileSync(resolve(".env"), "utf-8")
  const match = envContent.match(
    /^#?\s*DIRECT_DATABASE_URL="(postgresql:\/\/postgres:[^"]*@db\.zzdamokfeenencuggjjp[^"]*)"/m
  )
  if (!match) {
    console.error(
      "Could not find PoC DIRECT_DATABASE_URL in .env (project zzdamokfeenencuggjjp)"
    )
    process.exit(1)
  }
  // Remove schema= parameter -- we want the default public+auth schemas
  return match[1].replace(/\?schema=\w+/, "")
}
...
// Prisma reads from prod (via DATABASE_URL in .env.supabase)
...
async function fetchPocPasswordHashes(): Promise<Map<string, string>> {
  ...
  const pg = new PgClient({ connectionString: POC_DATABASE_URL })
  await pg.connect()

  const { rows } = await pg.query<{ id: string; encrypted_password: string }>(
    "SELECT id, encrypted_password FROM auth.users WHERE encrypted_password IS NOT NULL AND encrypted_password != ''"
  )
...
async function run() {
  ...
  console.log(`  PoC DB: db.zzdamokfeenencuggjjp.supabase.co\n`)

  // 1. Fetch password hashes from PoC
  ...
  const withHash = users.filter((u) => pocHashes.has(u.id))
  const withoutHash = users.filter((u) => !pocHashes.has(u.id))
  ...
  if (DRY_RUN) {
    ...
    for (const user of users) {
      const hasHash = pocHashes.has(user.id) ? "HAS_HASH" : "NO_HASH"
      console.log(
  ...
  for (let i = 0; i < users.length; i += BATCH_SIZE) {
    ...
    for (const user of batch) {
      const passwordHash = pocHashes.get(user.id)
      ...
      if (error) {
        if (
          ...
          console.error(`  ERROR: ${user.email} -> ${error.code}: ${error.message}`)
        }
      } else if (data?.user) {
        result.migrated++
      }
